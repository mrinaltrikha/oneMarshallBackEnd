...
@Component({
  selector: 'app-marshall-matters',
  templateUrl: './marshall-matters.component.html',
  styleUrls: ['./marshall-matters.component.css']
})
export class MarshallMattersComponent implements OnInit {
  ...
  // Create
  createRecord() {
    this.http.post('api/issue', this.recordBeingEdited).subscribe(data => {
  ...
  // Retrieve All
  refreshList() {
    this.http.get('api/issues').subscribe(data => {
  ...
  // Update
  updateRecord() {
    this.http.put('api/issue/' + this.recordBeingEdited_Id, this.recordBeingEdited).subscribe(data => {
      this.modalViewForRecordDetails_Ref.close();
      this.refreshList();
    });
  }
  ...
  // Delete
  deleteRecord(id) {
    this.http.delete('api/issue/' + id).subscribe(data => {
      this.refreshList();
    })
  }
  ...
  showEditRecordForm(id, modal) {
    this.http.get('api/issue/' + id).subscribe(data => {
